...
const columns: Array<{ key: string; label: string; value: (job: JobView) => string | number | boolean | null }> = [
  ...
  { key: "amplifyDb", label: "Gain Trim dB", value: (job) => job.amplifyDb },
  { key: "targetLufs", label: "Target LUFS", value: (job) => job.targetLufs },
  { key: "quality", label: "Quality", value: (job) => job.quality },
  { key: "audioSafetyMode", label: "Audio Safety Mode", value: (job) => job.audioSafetyMode },
  { key: "headroomDb", label: "Peak Limit dBFS", value: (job) => job.headroomDb },
  { key: "limiterEnabled", label: "Limiter + Normalize", value: (job) => job.limiterEnabled },
